...
export function createPreactComp ({ h }, {
  __stylt = styltFallback,
  compute = I
}) {
  const cachedProps = { [NO_ATTRS_STR]: __stylt.attrs }

  function PreactComp ({ children, ...props }) {
    const { styleProps, rest } = isEmpty(props) ? {} : sortAttrs(props)
    const stylePropsStr = styleProps ? JSON.stringify(styleProps) : NO_ATTRS_STR
    const $cached = cachedProps[stylePropsStr]
    const computed = $cached || compute(styleProps)

    return h(props.as || __stylt.tag, Object.assign(rest || {}, computed), children)
  }

  PreactComp.stylt = __stylt
  return PreactComp
}
