 ...
 */
export function createPrelimRes (parent, parsedThing, parse, fns, theme, output = 'class' || 'style') {
 ...
 * @return {function(*=)}
 */
export function preCompute (prelim, parse = b, fns = _blnkArr, theme, output = 'class' || 'style') {
  const result = {}
 ...
 * @return {function}
 */
export const createStyler = (parser = b, { m, preact, React, outputType = 'class', theme }) => {
  /**
  ...
  return function styler (a, b, ...c) {
    ...
    else throw Error('[Stylething createStyler] No renderer found in options!')
  }
}
