###### [Style Cookbook](../index.md)  >  [Typography](./index.md)

# Skins

_[TODO]_ change text:
...
Setting up BSS color helpers

Using [theme colors](../color.md)

```js
import b from 'bss'
import { dfaultTheme, createStyler } from 'stylething'
import m from 'mithril' // view layer
import { color } from 'systemthing'

const { colors } = dfaultTheme
const styled = createStyler(b, { m })

const BoxI = styled('div', b.padding('0.5em').color(colors.white50).bc(colors.darkRed))
const BoxII = styled('div', color)

const A = m(BoxI, 'Eqivalent to B')
const B = m(BoxII, {
  color: 'white50',
  backgroundColor: 'darkRed'
}, 'Eqivalent to A')
```
Also works for responsively styled components

```js
const BoxIII = styled('div', b`
  padding .5em
  color ${colors.white50}
  bc ${colors.darkRed}
`.$notSmall`
  color ${colors.washedYellow}
  bc ${colors.darkBlue}
`)
const BoxIV = styled('div', b`padding 0.5em`, color)

const C = m(BoxIII, 'Equivalent to D')
const D = m(BoxIV, {
  color: [ 'white50', 'washedYellow' ],
  backgroundColor: [ 'darkRed', 'darkBlue' ]
})
```

<!--Inspiration Tachions
