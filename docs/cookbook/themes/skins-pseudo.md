...
You can use any color from `theme.colors` to style text and background colors for hover and focus states.

Setting up BSS color helpers (limited utility)
...
const _hoverStyle = { color: colors.red, bc: colors.darkBlue }

const Box = styled('div', b.$hover(boxHover).$focus(boxHover))
```
